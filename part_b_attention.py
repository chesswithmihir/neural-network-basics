...
if __name__ == "__main__":
    # Create sample data
    # Batch size = 2, Sequence length = 4, Dimension = 3
    X_BTD = np.random.randn(2, 4, 3)
    
    # Initialize weight matrices
    W_q_DD = np.random.randn(3, 3)
    W_k_DD = np.random.randn(3, 3)
    W_v_DD = np.random.randn(3, 3)
    
    # Run attention
    output_BTD = self_attention(X_BTD, W_q_DD, W_k_DD, W_v_DD)
    
    print("Input shape:", X_BTD.shape)
    print("Output shape:", output_BTD.shape)
    print("Sample input:", X_BTD[0, 0])
    print("Sample output:", output_BTD[0, 0])